<?php
    ...
    $filiere = "ING : Génie maritime et portuaire";
    $db = Database::connect();
        ...
        <?php
            for($i = 1; $i <= $max ; $i++)
            {
                ...
                echo '          <div class="slides">';
                                    ...
                                    $statements = $db->prepare("SELECT * FROM parrain_marine WHERE id = ?");
                                ...
                                echo '<div class="slide special">
                                        ...
                                        for($j = 1; $j <= 30; $j++)
                                        {
                                            ...
                                            {
                                                $db = Database::connect();
                                                $statement = $db->query("SELECT image FROM parrain_marine WHERE id = ROUND(RAND()*((SELECT COUNT(*) FROM parrain_marine)-1))+1");
                                        ...
                                        <div class="thumbnail">
                                            <div class="slides">
                                                ...
                                                for($j = 1; $j <= 30; $j++)
                                                {
                                                    ...
                                                    {
                                                        $db = Database::connect();
                                                        $statement = $db->query("SELECT image FROM filleul_marine WHERE id = ROUND(RAND()*((SELECT COUNT(*) FROM filleul_marine)-1))+1");
                                                        $filleul = $statement->fetch();
                                                        if($filleul == false)
                                                        {
                                                            $isSuccess = false;
