<?php
    ...
    $filiere = "SCI-ING : géophysique,eau et environnement";
    $db = Database::connect();
        ...
        <?php
            for($i = 1; $i <= $max ; $i++)
            {
                ...
                echo '          <div class="slides">';
                                    ...
                                    $statements = $db->prepare("SELECT * FROM parrain_geophy WHERE id = ?");
                                ...
                                echo '<div class="slide special">
                                        ...
                                        for($j = 1; $j <= 30; $j++)
                                        {
                                            ...
                                            {
                                                $db = Database::connect();
                                                $statement = $db->query("SELECT image FROM parrain_geophy WHERE id = ROUND(RAND()*((SELECT COUNT(*) FROM parrain_geophy)-1))+1");
                                        ...
                                        <div class="thumbnail">
                                            <div class="slides">
                                                ...
                                                for($j = 1; $j <= 30; $j++)
                                                {
                                                    ...
                                                    {
                                                        $db = Database::connect();
                                                        $statement = $db->query("SELECT image FROM filleul_geophy WHERE id = ROUND(RAND()*((SELECT COUNT(*) FROM filleul_geophy)-1))+1");
                                                        $filleul = $statement->fetch();
                                                        if($filleul == false)
                                                        {
                                                            $isSuccess = false;
