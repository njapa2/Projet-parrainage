<?php
    ...
    $filiere = "SCI-ING III SDIA : Science des données et Intelligence  Artificielle";
 ...
 if(!empty($_POST))
 {
    ...
    if($isSuccess && $isUploadSuccess)
    {
        ...
        while ($parrainFind == false)
        {
            ...
            $db = Database::connect();
            $esssaie = $db->prepare("SELECT * FROM parrain_sdia WHERE id = ?");
        ...
        $nbre_filleul += 1;
        $db = Database::connect();
        $statement = $db->prepare("INSERT INTO filleul_sdia (matricule,nom,prenom,numero_parrain,image,Email,numero) VALUES (? , ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($matricule,$nom,$prenom,$numParrain,$image,$emailFilleul,$numero));
        $nombre_filleul = $db->prepare("UPDATE parrain_sdia SET nbre_filleul = ? WHERE id = ?");
...
<body>
    <section id="filleul">
        ...
        <p style="font-size: 20px;">Filière <?php echo $filiere; ?></p>
            <div class="row">
                  <form class="form" action="<?php echo 'filleul_sdia.php'; ?>" role="form" id="monform" method="post" enctype="multipart/form-data">
                    <div class="col-sm-6">
