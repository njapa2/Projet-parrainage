<?php
    ...
    $filiere = "ING III : Génie automobile et mécatronique";
 ...
 if(!empty($_POST))
 {
    ...
    if($isSuccess && $isUploadSuccess)
    {
        ...
        {
            $numParrain = rand(1,10);
            $db = Database::connect();
            $esssaie = $db->prepare("SELECT * FROM parrain_automeca WHERE id = ?");
        ...
        $nbre_filleul += 1;
        $db = Database::connect();
        $statement = $db->prepare("INSERT INTO filleul_automeca (matricule,nom,prenom,numero_parrain,image,Email,numero) VALUES (? , ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($matricule,$nom,$prenom,$numParrain,$image,$emailFilleul,$numero));
        $nombre_filleul = $db->prepare("UPDATE parrain_automeca SET nbre_filleul = ? WHERE id = ?");
...
<body>
    <section id="filleul">
        ...
        <p style="font-size: 20px;">Filière <?php echo $filiere; ?></p>
            <div class="row">
                  <form class="form" action="<?php echo 'filleul_automeca.php'; ?>" role="form" id="monform" method="post" enctype="multipart/form-data">
                    <div class="col-sm-6">
